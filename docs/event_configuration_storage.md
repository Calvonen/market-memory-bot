...
- **persisted**: `POST /api/v1/events/{event_id}/strategy-draft/approve` re-validates the exact same draft, then checks three independent conditions before persisting anything:
  ...
  `StrategyDraftPayload.consensus`/`.triggers` (and the admin write endpoint's `ExpectationVersionRequest.consensus`/`.triggers`) reject any non-finite number - `Infinity`, `-Infinity`, `NaN` - via `reject_non_finite_numbers()` (`trading_system/strategy_draft.py`), a `field_validator(mode="after")` on both models. This is a backend-boundary check, not something the mobile app's own JSON editor (`mobile/src/utils/strategy-draft-format.ts`) can substitute for: preview/approve/the admin write endpoint are a control API any trusted caller can drive directly, not only the mobile app. Pydantic's own `float` validation does not reject any of the three by default (`allow_inf_nan` defaults to true); a request body doesn't need an explicit `Infinity`/`NaN` token to produce one either, since Python's `json` module (what FastAPI/Starlette parse request bodies with) accepts those two as a non-standard extension on the way in, and a syntactically ordinary number literal can still numerically overflow to `Infinity` when parsed (e.g. `"1e400"`) - `typeof`-equivalent type checks alone cannot distinguish that from any other finite `float`. Without this check, a non-finite value would have reached `draft_fingerprint()`'s `json.dumps()` call (which is exactly as permissive on the way back out, so it would have re-emitted a non-standard `Infinity`/`NaN` token into what is meant to be the canonical JSON the fingerprint hashes) and the eventual Postgres `jsonb` write (which has no representation for any of the three at all, so that write would have failed as a raw, unmapped database error instead of a clean validation response).

  Rejecting the value is not sufficient on its own: FastAPI's default `RequestValidationError` handler echoes the rejected value straight back in the 422 response body (Pydantic's `"input"` field in each error detail), and Starlette's `JSONResponse` renders with `allow_nan=False` (unlike `json.dumps()`'s own permissive default) - so without further handling, the very 422 response reporting "this value is invalid" would itself raise an unhandled `ValueError` while trying to serialize the value it's rejecting. `create_app()` (`trading_system/api.py`) registers a custom `RequestValidationError` handler that mirrors FastAPI's default response shape but passes the encoded error body through `_json_safe()` first - a small recursive helper that replaces any non-finite float with its `str()` form (e.g. `"inf"`) before Starlette ever tries to encode it.
...
The direct admin write endpoint (`POST .../expectation-versions`) and the strategy-draft approval endpoint both allocate the next `event_expectation_versions` row for an event. They now both take the *same* `pg_advisory_xact_lock(hashtextextended(event_id, 1))` before doing so - the admin path through the `insert_next_expectation_version()` Postgres function (`supabase/migrations/20260821140000_shared_expectation_version_lock.sql`), called from `SupabaseEventExpectationRepository.apply_partial_update()` instead of that method's old, unlocked "select max(version), insert, retry on 23505" loop. A concurrent admin write and approval for the same event can therefore never race each other's version allocation: whichever acquires the lock first fully commits before the other's read-then-insert proceeds. The admin path has no caller-supplied expected version to check (it has never offered CAS semantics - it just writes on top of whatever is current), so it always succeeds once it holds the lock; only the approval path can conflict, and it does so as a controlled `ExpectationVersionConflict` -> 409, never an unmapped 500 (`SupabaseStrategyDraftApprovalRepository._is_version_conflict()` also treats a raw `23505` as this same conflict, as defense in depth, in case some future writer is ever added without taking the lock). The in-memory test doubles mirror this by sharing one `threading.Lock` (`InMemoryEventExpectationRepository.lock`, reused by `InMemoryStrategyDraftApprovalRepository`).

Taking the same lock is not, on its own, enough to stop the admin endpoint from reverting fields an approval just changed - the lock only serializes *when* each writer's read-then-insert runs, not *what* it reads from. The admin endpoint's `ExpectationVersionRequest` is a partial patch (every field but `change_note` is optional, meaning "leave unchanged"); the version this used to send to the repository was already a fully-merged `EventExpectation`, built in `create_expectation_version()` (`trading_system/api.py`) from a `repo.get()` call made *before* the lock was ever acquired. If a strategy-draft approval committed a change to some field the admin patch never mentioned - in the window between that pre-lock read and the eventual locked write - the admin write would still insert the pre-approval value for that field once it finally ran, silently undoing the approval's change without either endpoint ever reporting a conflict.

Fixing this meant moving the merge itself behind the lock, not just the write: `create_expectation_version()` now builds an unresolved `ExpectationPatch` (`trading_system/event_repository.py`) straight from the request - with `None` on every field the admin didn't set - and hands it directly to `EventExpectationRepository.apply_partial_update()` without ever calling `repo.get()` itself. `InMemoryEventExpectationRepository.apply_partial_update()` reads `current` from `self.events` *inside* the same `with self.lock:` block it writes the merged result in, so no other writer sharing that lock can commit in between. `insert_next_expectation_version()` does the equivalent in Postgres: it takes nullable partial-patch parameters (same parameter list/types as the version that first introduced the lock - only their meaning changed), reads the full current `event_expectation_versions` row itself *after* calling `pg_advisory_xact_lock()`, `coalesce()`s each `NULL` input against that freshly-read row, and inserts the merged next version - all inside its own single transaction. An event that doesn't exist is detected inside that same locked read (`event_not_found` / `P0002`, the same convention `approve_strategy_draft()` already uses) and mapped to `EventExpectationNotFound` -> 404, rather than by a separate `repo.get()` check beforehand.

**The RPC's response is the only source of truth for what it just wrote - never a follow-up read.** An earlier version of this fix had `SupabaseEventExpectationRepository.apply_partial_update()` call `get()` again immediately after the RPC's transaction committed, to build the full `EventExpectation` its caller needs back (`event_expectation_versions` has no `instrument`/`event_name`/`scheduled_date` columns of its own - those live on `market_events`). That re-read was itself a separate, unlocked query: if a second writer (another admin patch, or a strategy-draft approval) committed a *later* version for the same `event_id` in the gap between this call's own transaction committing and that re-read running, the re-read would return the later writer's row - so this call's response would describe someone else's write as if it were its own. `insert_next_expectation_version()` now returns every field a caller needs - all the merged versioned fields plus the event's identity, joined from `market_events` inside the same transaction - directly in its own return row (columns prefixed `out_` to avoid plpgsql's `#variable_conflict` ambiguity with the real column names being selected from within the function body), built from the exact same `merged_*` local variables the `insert` statement itself used. `SupabaseEventExpectationRepository.apply_partial_update()` now builds the returned `EventExpectation` entirely from that row - it makes no `.table(...)` call of any kind.

### A same-signature RPC body change needs its own migration and a version marker

`insert_next_expectation_version()`'s call signature has stayed identical across several rounds of changes to its actual behavior - first to take the lock at all, then to resolve its patch against a post-lock read instead of a pre-lock one, then to return the full written row instead of just `(new_version, created_at)`. Editing the function body in place (`create or replace` inside the migration that first declared it) is fine for local iteration, but it is exactly what a signature-only deploy gate cannot detect: `to_regprocedure()` reports whether a function with a given signature exists, never what its body currently does. A Supabase project that had only an *earlier* version of `insert_next_expectation_version()` applied - identical signature, outdated semantics - would pass every existence check the gate used to run, and the backend would be restarted into production still calling a function whose real behavior no longer matches what the backend code assumes.

Two changes close this gap, both introduced in `supabase/migrations/20260823090000_expectation_write_atomic_response_and_schema_version.sql`:

1. **New migration, not another in-place edit.** This file is the first to actually change `insert_next_expectation_version()`'s semantics as its own migration, rather than editing an earlier one - the earlier migrations (`20260821140000_shared_expectation_version_lock.sql`, `20260822090000_verify_strategy_draft_schema.sql`) are marked `SUPERSEDED` at the top and kept unedited as the historical record of when the lock, and the original three-check gate, were first introduced.
2. **`public.strategy_draft_schema_version()`** - a trivial, `immutable` SQL function returning a single hardcoded integer, bumped by any future migration that changes this write path's semantics again. `verify_strategy_draft_schema()` now also returns `schema_version_matches` (`strategy_draft_schema_version() = <the exact value this backend commit requires>`), and `scripts/verify_supabase_schema.py`'s `REQUIRED_CHECKS` fails closed on it exactly like every other check. A Supabase project missing this migration either has no `strategy_draft_schema_version()` to call at all (which fails the whole RPC call - the same fail-closed path as any other not-yet-applied migration) or has it returning a stale value - either way, `schema_version_matches` is false or absent, and the gate refuses to let the deploy proceed. A same-signature RPC from an older migration can therefore never silently satisfy a version the backend actually requires.
...
**The deploy workflow still verifies this deterministically, every time - it does not rely on remembering to check manually.** The "Deploy backend to seesam-hub (locked)" step now runs `scripts/verify_supabase_schema.py` after fast-forwarding the checkout but *before* either `systemctl restart` line. That script calls the read-only `verify_strategy_draft_schema()` RPC, which checks `to_regclass()`/`to_regprocedure()` catalog existence for the table/functions this write path depends on, **plus** `schema_version_matches` - whether `strategy_draft_schema_version()` (a small marker function bumped by every migration that changes this write path's semantics under an unchanged call signature; see "A same-signature RPC body change needs its own migration and a version marker" above) equals the exact value this backend commit requires. No data is read or written by any of these checks, and calling them repeatedly is always safe. If the RPC call fails outright (almost always meaning the migrations haven't been applied yet - including the case where the verify RPC, or `strategy_draft_schema_version()`, is itself missing) or any individual check comes back false, the script exits non-zero.

GitHub Actions `run:` steps use `bash -eo pipefail` by default, so that non-zero exit stops the step immediately: neither `systemctl restart marketai-api.service` nor `systemctl restart marketai-hays-release.service` runs, and the currently-running (older) backend keeps serving traffic untouched - **fail closed**, never a bad restart. This applies to *every* push to `feature/trading-system-foundation`, not just this PR's schema - any future PR that changes an existing RPC's semantics under an unchanged signature must bump `strategy_draft_schema_version()` in its own new migration (never by re-editing an already-applied one) and update the expected value `verify_strategy_draft_schema()` compares it against; a PR that adds an entirely new Supabase dependency should extend `verify_strategy_draft_schema()` with a new existence check and `scripts/verify_supabase_schema.py`'s `REQUIRED_CHECKS` the same way.
...
`parseTypedRecord()` also rejects any numeric value that isn't `Number.isFinite()` - `Infinity`, `-Infinity`, and `NaN` are all invalid consensus/trigger values, shown as a validation error rather than silently accepted. JSON itself has no literal syntax for any of those three, but a syntactically ordinary-looking number literal can still overflow to `Infinity` when parsed (e.g. `1e400`, or its negative form), and `typeof` still reports that result as `'number'` - so without this check it would sail past the existing `typeof raw !== 'number'` guard undetected. Accepting it would have been a real, silent data-loss bug of its own: `JSON.stringify(Infinity)` renders as the text `null`, so an accepted `1e400` would quietly turn into an actual `null` value the next time that same record round-trips back through `recordToText()` - a value the user never typed, replacing one they did.
