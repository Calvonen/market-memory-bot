...
- **persisted**: `POST /api/v1/events/{event_id}/strategy-draft/approve` re-validates the exact same draft, then checks three independent conditions before persisting anything:
  1. **Draft-fingerprint check** - the draft in the approval request must hash to the same `draft_fingerprint` the preview returned, so an approved payload can never silently differ from what was reviewed. `StrategyDraftApprovalRequest.draft_fingerprint` is validated as exactly 64 `[0-9a-fA-F]` characters (`Field(min_length=64, max_length=64, pattern=...)`) before this check ever runs - `secrets.compare_digest()` raises `TypeError` on a non-ASCII string argument, so a malformed fingerprint used to be able to crash the request as an unhandled 500 instead of the 422 a bad request should produce.
...
`mobile/src/utils/strategy-draft-format.ts`'s consensus/trigger editor round-trip is fully lossless, not heuristic. `recordToText()` renders every value as a JSON literal (`JSON.stringify`) - numbers and `null` bare, strings always quoted - and `textToTypedRecord()` (shared by both `consensus` and `triggers`) parses each value back with `JSON.parse`, falling back to the literal trimmed text only when that fails. This is what makes the string `"001"` stay a string (leading zeros make it invalid JSON number syntax, so it can never be coerced to the number `1`), the string `"null"` stay a string (it only round-trips back to that exact string because it was rendered quoted), real `null` stay `null`, and a real number stay a number - with no `Number(value)`/`isFinite` guessing anywhere in the parser.
