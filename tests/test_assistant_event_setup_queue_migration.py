...
class AssistantEventProposalStorageMigrationTests(unittest.TestCase):
    ...
    def test_reviewed_content_is_immutable(self) -> None:
    ...
    def test_proposal_id_is_always_immutable(self) -> None:
        self.assertIn("if new.id is distinct from old.id then", self.sql)
        self.assertIn("assistant_proposal_id_is_immutable", self.sql)

    def test_reviewer_metadata_is_written_once_and_then_frozen(self) -> None:
        self.assertIn("old.status = 'ready_for_review'", self.sql)
        self.assertIn("new.status in ('approved_for_materialization', 'rejected')", self.sql)
        self.assertIn("assistant_proposal_review_metadata_required", self.sql)
        self.assertIn("assistant_proposal_review_metadata_is_immutable", self.sql)
