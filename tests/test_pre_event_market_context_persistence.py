...
class FailPreEventDeadlineIfCurrentTests(unittest.TestCase):
    def test_calls_cas_rpc_with_exact_version_actor_and_error(self) -> None:
        repository = _Repository(event=object())
        version = datetime(2026, 8, 24, 14, 47, tzinfo=UTC)
            ...
            [
                (
                    "fail_tracked_market_event_pre_event_deadline_if_current",
                    {
                        ...
                        "input_actor": "tracked-event-worker",
                        "input_error": (
                            "event reached event_at before pre-event market context was prepared"
                        ),
                    },
                )
            ],
