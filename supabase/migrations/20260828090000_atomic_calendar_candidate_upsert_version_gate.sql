-- Couple the placeholder-preserving candidate upsert implementation to the
-- version marker used by the deploy gate. Earlier follow-up migrations could
-- be applied independently, allowing the marker to attest to an old body.
-- Transactional DDL makes the body, marker, and verifier inseparable.
begin;
