-- Durable assistant-proposal lineage and reviewed-snapshot enforcement.
--
-- This migration closes three gaps found in review of the first Syrah DEMO
-- end-to-end safety fix:
--   * assistant origin must not be inferred from the shared (manual, external_key)
--     namespace;
--   * the strategy finalizer must not trust caller-selected event/strategy/source
--     values as proof of reviewed intent;
--   * timezone-less event_at must fail at the SQL boundary.
--
...
create table if not exists public.assistant_proposal_materializations (
  proposal_id uuid primary key
    references public.assistant_event_proposals(id) on delete restrict,
  review_round integer not null check (review_round > 0),
  tracked_event_id uuid not null unique
    references public.tracked_market_events(id) on delete restrict,
...
revoke all on table public.assistant_proposal_materializations
...
revoke all on function public.assistant_normalize_text_array(jsonb)
...
begin
  ...
  if review_row.proposal_key is distinct from input_external_key
     or nullif(btrim(review_row.payload ->> 'company_name'), '')
        is distinct from nullif(btrim(input_company_name), '')
     or upper(btrim(review_row.payload ->> 'instrument'))
        is distinct from upper(btrim(input_instrument))
     or nullif(btrim(review_row.payload ->> 'market'), '')
        is distinct from nullif(btrim(input_market), '')
     or review_row.payload ->> 'kind' is distinct from input_kind
     or nullif(btrim(review_row.payload ->> 'title'), '')
        is distinct from nullif(btrim(input_title), '')
     or (review_row.payload ->> 'scheduled_date')::date is distinct from input_event_date
     or (review_row.payload ->> 'base_expectation_version')::integer
        is distinct from input_expected_base_version then
  ...
  -- timestamptz would otherwise accept a timezone-less value using the session
  -- timezone. The reviewed text itself must carry an explicit UTC/offset suffix.
  ...
  if existing_event_count = 1 then
    ...
    -- Never adopt an existing unbound ordinary manual event merely because its
    -- external key happens to match a proposal key.
    if not found then
      raise exception 'assistant_proposal_manual_identity_collision' using errcode = '55000';
    end if;
    if binding.proposal_id is distinct from input_proposal_id then
  ...
  insert into public.assistant_proposal_materializations (
    ...
    input_proposal_id,
    input_expected_review_round,
    canonical_write.out_id,
    'tracked:' || canonical_write.out_id::text,
    clock_timestamp()
  )
  ...
  if not found
     ...
     or binding.tracked_event_id is distinct from canonical_write.out_id
     or binding.event_id is distinct from ('tracked:' || canonical_write.out_id::text) then
    raise exception 'assistant_proposal_materialization_binding_conflict' using errcode = '55000';
  end if;

  return query select
    canonical_write.out_id,
    canonical_write.out_tracked_instrument_id,
    canonical_write.out_calendar_event_id,
    canonical_write.out_company_name,
    canonical_write.out_instrument,
    canonical_write.out_market,
    canonical_write.out_source,
    canonical_write.out_external_key,
    canonical_write.out_kind,
    canonical_write.out_title,
    canonical_write.out_event_at,
    canonical_write.out_event_date,
    canonical_write.out_event_time_status,
    canonical_write.out_status,
    canonical_write.out_reference_price,
    canonical_write.out_reference_captured_at,
    canonical_write.out_created_by,
    canonical_write.out_updated_by,
    canonical_write.out_created_at,
    canonical_write.out_updated_at,
    canonical_write.out_action;
end;
$$;
...
begin
  ...
  from public.tracked_market_events
  where id = binding.tracked_event_id
  for update;

  if not found
     or tracked_row.source is distinct from 'manual'
     or tracked_row.external_key is distinct from review_row.proposal_key
     or upper(btrim(tracked_row.instrument))
        is distinct from upper(btrim(review_row.payload ->> 'instrument'))
     or nullif(btrim(tracked_row.company_name), '')
        is distinct from nullif(btrim(review_row.payload ->> 'company_name'), '')
     or nullif(btrim(tracked_row.market), '')
        is distinct from nullif(btrim(review_row.payload ->> 'market'), '')
     or tracked_row.kind is distinct from review_row.payload ->> 'kind'
     or nullif(btrim(tracked_row.title), '')
        is distinct from nullif(btrim(review_row.payload ->> 'title'), '')
     or tracked_row.event_date is distinct from (review_row.payload ->> 'scheduled_date')::date
     or tracked_row.event_time_status is distinct from review_row.payload ->> 'event_time_status'
     or tracked_row.event_at is distinct from (review_row.payload ->> 'event_at')::timestamptz then
    raise exception 'assistant_proposal_canonical_event_drift' using errcode = '55000';
  end if;
  ...
  if (review_row.payload ->> 'base_expectation_version')::integer
       is distinct from input_expected_base_version
     or nullif(btrim(strategy ->> 'source_name'), '')
       is distinct from nullif(btrim(input_source_name), '')
     or nullif(btrim(strategy ->> 'source_url'), '')
       is distinct from nullif(btrim(input_source_url), '')
     ...
     or public.assistant_normalize_text_array(coalesce(strategy -> 'important_kpis', '[]'::jsonb))
       is distinct from input_important_kpis
     or public.assistant_normalize_text_array(coalesce(strategy -> 'bull_case', '[]'::jsonb))
       is distinct from input_bull_case
     or public.assistant_normalize_text_array(coalesce(strategy -> 'base_case', '[]'::jsonb))
       is distinct from input_base_case
     or public.assistant_normalize_text_array(coalesce(strategy -> 'bear_case', '[]'::jsonb))
       is distinct from input_bear_case
     or coalesce(strategy -> 'triggers', '{}'::jsonb) is distinct from input_triggers
     or public.assistant_normalize_text_array(coalesce(strategy -> 'invalidation_conditions', '[]'::jsonb))
       is distinct from input_invalidation_conditions
  ...
  expected_source_actor := 'assistant_proposal:' || input_proposal_id::text ||
    ':reviewer:' || review_row.reviewed_by;
...
revoke all on function public.approve_assistant_proposal_strategy_draft(
  uuid, integer, text, integer, text, text, date, jsonb, jsonb, jsonb, jsonb,
  jsonb, jsonb, jsonb, text, text, text, text, text, text, text, integer,
  boolean, text
) from public, anon, authenticated;
...
begin
  ...
  if binding.proposal_id is not null
     and receipt_expectation_version is distinct from current_version then
...
revoke all on function public.approve_paper_trading_task_for_event(
  uuid, text, text, text, integer, numeric
) from public, anon, authenticated;
grant execute on function public.approve_paper_trading_task_for_event(
  uuid, text, text, text, integer, numeric
) to service_role;
