-- Keep cancellation in the same event-lock order as claim/revalidation so it
-- cannot deadlock while also preventing cancellation under an active lease.
...
begin
  ...
  -- Same fixed order as task-aware claim and broker revalidation.
