-- Publish a version that is distinct from the earlier marker-only gate, while
-- installing the implementation it attests to in the same transaction.
begin;
...
as $$
  select
    ...
    public.calendar_candidate_upsert_version() = 2,
