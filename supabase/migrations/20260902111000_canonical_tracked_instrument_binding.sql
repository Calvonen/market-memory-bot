-- Bind producer-supplied tracked-instrument identity inside the canonical
-- tracked-event database transaction. The previous wrapper could only return
-- the randomly generated tracked_instrument_id from the legacy writer, which
-- made a post-write Python identity check fail after the event had committed.
begin;
...
begin
  ...
  if input_expected_tracked_instrument_id is not null
     and btrim(input_expected_tracked_instrument_id) = '' then
    raise exception 'tracked_market_event_expected_instrument_id_invalid';
  end if;
  ...
  -- The legacy writer generates a tracked_instrument_id for a fresh row. For a
  -- producer-neutral canonical write, replace that generated value inside this
  -- same transaction before anything is committed. Existing rows must already
  -- belong to the requested canonical tracked instrument or the whole RPC rolls
  -- back, including any update performed by the legacy writer above.
  if input_expected_tracked_instrument_id is not null then
    if upserted.out_action = 'inserted' then
      if saved_row.tracked_instrument_id is distinct from input_expected_tracked_instrument_id then
    ...
    elsif saved_row.tracked_instrument_id is distinct from input_expected_tracked_instrument_id then
      raise exception 'tracked_market_event_instrument_binding_conflict';
    end if;
  end if;
