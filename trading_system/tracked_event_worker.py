...
async def monitor_one_event(
    ...
    *,
    ...
    # Must happen before any timing-critical step below (reference capture,
    # market-open wait, reaction monitoring) - a restart with an already
    # identical snapshot continues normally (idempotent), but a conflicting
    # snapshot or a capture failure fails this event closed rather than
    # monitoring it with settings that were never durably audited.
    try:
